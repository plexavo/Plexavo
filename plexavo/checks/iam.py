...
def _apply_deny_and_boundary(principal: Principal, action: str, resource_arn: str | None,
                              severity: Severity, confidence: str, evidence: str):
    ...
    Returns (severity, confidence, evidence) or None if the finding should
    be suppressed."""
    ...
    if blocked and is_conditioned:
        ...
        extra = "May also be blocked by a conditioned Deny elsewhere in this principal's policies."
    ...
    for p in principals:
        for policy_name, stmt in _all_statements(p):
            ...
            if statement_has_wildcard_action(stmt) and resource_is_wildcard(stmt):
                ...
                result = _apply_deny_and_boundary(p, "*", None, severity, confidence, evidence)
                if result is None:
                    continue
    ...
    for p in principals:
        for policy_name, stmt in _all_statements(p):
            ...
            for action in granted:
                result = _apply_deny_and_boundary(p, action, target_resource, base_severity, base_confidence, base_evidence)
                if result is None:
                    continue  # this specific action is unconditionally blocked
                _, action_confidence, _ = result
                if action_confidence != base_confidence:
                    any_conditioned_deny = True
                remaining.add(action)
            if not remaining:
                continue  # every granted action was unconditionally denied

            severity, confidence, evidence = base_severity, base_confidence, base_evidence
            if any_conditioned_deny:
                severity = Severity.HIGH if severity == Severity.CRITICAL else severity
                confidence = "Likely — see note"
                extra = "Some of this access may additionally be blocked by a conditioned Deny on one or more of the listed actions."
                evidence = f"{evidence} {extra}".strip()
            ...
            findings.append(Finding(
                ...
                raw_detail=f"Policy '{policy_name}' on {p.type} '{p.name}' grants "
                           ...
                           f"additional permission or credential.",
                account_context=f"policy={policy_name}",
                confidence=confidence,
                evidence=evidence,
            ))
    ...
    for p in principals:
        for policy_name, stmt in _all_statements(p):
            ...
            if resource_is_wildcard(stmt):
                result = _apply_deny_and_boundary(p, "iam:CreatePolicyVersion", None, severity, confidence, evidence)
                if result is None:
                    continue
                severity, confidence, evidence = result
                findings.append(Finding(
                    ...
                    raw_detail=f"Policy '{policy_name}' on {p.type} '{p.name}' grants "
                               ...
                               f"itself full access and set it as the default version.",
                    account_context=f"policy={policy_name}",
                    confidence=confidence,
                    evidence=evidence,
                ))
                ...
                for r in _normalize(stmt.get("Resource")):
                    ...
                    if owner is None or owner == p.arn:
                        continue
                    result = _apply_deny_and_boundary(p, "iam:CreatePolicyVersion", r, severity, confidence, evidence)
                    if result is None:
                        continue
                    r_severity, r_confidence, r_evidence = result
                    findings.append(Finding(
                        check_id="IAM-03",
                        title="CreatePolicyVersion Escalation",
                        severity=r_severity,
    ...
    for p in principals:
        for policy_name, stmt in _all_statements(p):
            ...
            for role_arn, role_name in reachable.items():
                if role_arn == p.arn:
                    continue
                result = _apply_deny_and_boundary(p, "iam:PassRole", role_arn, base_severity, base_confidence, base_evidence)
                if result is None:
                    continue
                severity, confidence, evidence = result
                findings.append(Finding(
                    ...
                    raw_detail=f"{p.type} '{p.name}' has iam:PassRole (policy '{policy_name}') "
                               ...
                               f"gaining that role's permissions.",
        ...
        for policy_name, stmt in _all_statements(p):
            ...
            findings.append(Finding(
                ...
                raw_detail=f"Policy '{policy_name}' on {p.type} '{p.name}' grants "
                           ...
                           f"admin roles created after this scan.",
                account_context=f"policy={policy_name}",
                confidence=confidence,
                evidence=evidence,
            ))
